...
export function buildSignalData(data, formData, type) {
    ...
    if (type === "Engine") {
        ...
        if (formData["Peak_Pressure_SENSORx"] !== "No Channel") {
            secondChannelName = "Peak Pressure - RPM Signal";
        }
    }
    if (type === "Torque") {
        ...
        if (type === "Engine") {
            ...
            for (const key in item) {
                if (keysToIgnore[type].includes(key)) {
                    ...
                    //const pressureValue = objData.value;
                    ...
                    if (key === "Pressure") {
                        ...
                        if (objData && objData.hasOwnProperty("cylinderValues")) {
                            ...
                            // Use the pressureValue as needed (for example, add it to the resultSet)
                        const pressureLabel = "Pressure"; // You may adjust the label as needed
                        const foundPressureIndex = resultSet.findIndex((x) => x.key === pressureLabel);

                        if (foundPressureIndex !== -1) {
                            resultSet[foundPressureIndex].data.push(pressureValue);
                        } else {
                            if (pressureValue > 0) {
                            resultSet.push({
                               key: pressureLabel,
